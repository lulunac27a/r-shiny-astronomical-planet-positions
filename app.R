...
ui <- fluidPage(titlePanel("Astronomical Planet Positions with Rise and Set Times and Sun"),
    sidebarLayout(sidebarPanel(sliderInput("date", "Day of Year",
        min = 1, max = 365, value = 1, step = 1), sliderInput("time",
        "Time of Day", min = 0, max = 24, value = 12, step = 0.01),
        sliderInput("latitude", "Latitude", min = -90, max = 90,
            value = 0, step = 0.01), sliderInput("elongation",
            "Elongation", min = -180, max = 180, value = 0, step = 0.01)),
...
server <- function(input, output) {
    day_to_month_day <- function(day) {
        ...
        days_in_month <- c(31, 28, 31, 30, 31, 30, 31, 31, 30,
            31, 30, 31)  #number of days in a month
        month_names <- c("January", "February", "March", "April",
            "May", "June", "July", "August", "September", "October",
            "November", "December")  #full month names
        month <- 1  #set initial month to 1 (January)
        while (day > days_in_month[month]) {
            # while day is greater than days in a specified
            # month
            day <- day - days_in_month[month]  #subtract days by days in a specified month
            month <- month + 1  #increase month by 1
        }
        return(list(month = month_names[month], day = day))  #return list of month and day number
    }

    sun_longitude <- function(date) {
        # get sun longitude based on day of year (date)
        return((date - 80) * 360/365)  #in degrees
    }
    ...
    planet_longitude <- function(date, elongation) {
        ...
        return((date - 80) * 360/365 + elongation)  #in degrees
    }
    ...
    planet_declination <- function(date, elongation) {
        ...
        return(asin(sin(23.44 * pi/180) * sin(planet_longitude(date,
            elongation) * pi/180)) * 180/pi)  #in degrees
    }
    ...
    planet_rise_set <- function(latitude, declination, elongation) {
        ...
        sun_right_ascension <- longitude_to_right_ascension(sun_longitude(input$date))  #in degrees
        ...
        rise <- (12 - hour_angle/15 - (sun_right_ascension -
            planet_right_ascension)/360 * 24)%%24  #in hours
    ...
    time_to_hh_mm <- function(time) {
        ...
        sign <- ifelse(time < 0, "-", "")  #add a minus sign if time is negative
        time <- abs(time)  #use absolute value of time
        hours <- floor(time)
    ...
    output$dateTime <- renderText({
        ...
        date <- input$date  #date input
        time <- input$time  #time input
    ...
    output$sunRiseSet <- renderText({
        ...
        date <- input$date
        latitude <- input$latitude  #latitude input
    ...
    output$planetPosition <- renderText({
        ...
        date <- input$date
        elongation <- input$elongation  #planet elongation input
    ...
    output$planetAltitudeAzimuth <- renderText({
        ...
        time <- input$time + 24/360 * (longitude_to_right_ascension(planet_longitude(date,
            input$elongation)) - longitude_to_right_ascension(sun_longitude(date)))  #adjust time based on planet right ascension and sun right ascension
        latitude <- input$latitude
        elongation <- input$elongation
        declination <- planet_declination(date, elongation)
        alt_az <- altitude_azimuth(latitude, declination, time)
